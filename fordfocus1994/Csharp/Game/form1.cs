    ...
    public partial class Form1 : Form
    {
        ...
        public void pictureBox1_MouseDown(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
            {
                foreach (Goblin g in G)
                {
                    if ((e.X >= g.X && e.X < g.X + 32) && (e.Y >= g.Y && e.Y < g.Y + 32))
                    {
                        TargetPicture.Image = Image.FromFile(@"D:\Proga\GameGraphics\GameGraphics\obj\Debug\GamePictures\fghtr2l.bmp");
                    }
                }
        ...
        private void CreationMap_Click(object sender, EventArgs e)
        {
            ...
            for (j = 0; j < MapHeight; j++)
            {
                for (i = 0; i < MapWidth; i++)
                {
                    MainTextBox.Text += MassOfMapObjects[i, j].ToString("n") + "\t";
                }
            ...
            if (G.Count < 10)
            {
                ...
                foreach (Goblin g in G)
                {
                    rollx = rand.Next(1, 25);
                    rolly = rand.Next(1, 13);
                    g.X = 9 + 50 * rollx;
                    g.Y = 9 + 50 * rolly;
                    rollx = 0;
                    rolly = 0;
           ...
           /* for (j = 0; j < 10; j++)
            {
                for (i = 0; i < 30; i++)
                {
                    if ((GobCoordsX[j] == TreeCoordsX[i] || GobCoordsX[j] == RockCoordsX[i]) && (GobCoordsY[j] == TreeCoordsY[i] || GobCoordsY[j] == RockCoordsY[i]))
                    {
                        GobCoordsX[j] = GobCoordsX[j] + 50;
                        GobCoordsY[j] = GobCoordsY[j] + 50;
                    }
                    if ((TreeCoordsX[j] == GobCoordsX[j] || TreeCoordsX[j] == RockCoordsX[i]) && (TreeCoordsY[j] == GobCoordsY[j] || TreeCoordsY[j] == RockCoordsY[i]))
                    {
                        TreeCoordsX[j] = TreeCoordsX[j] + 50;
                        TreeCoordsY[j] = TreeCoordsY[j] + 50;
                    }
                    if ((RockCoordsX[j] == TreeCoordsX[i] || RockCoordsX[j] == GobCoordsX[j]) && (RockCoordsY[j] == TreeCoordsY[i] || RockCoordsY[j] == GobCoordsY[j]))
                    {
                        RockCoordsX[j] = RockCoordsX[j] + 50;
                        RockCoordsY[j] = RockCoordsY[j] + 50;
                    }
                }
        ...
        private void LoadMap_Click(object sender, EventArgs e)
        {
            MassOfMapObjects = new int[MapWidth, MapHeight];
            int i, j, t, Buf = 0;
            using (StreamReader Reader = new StreamReader("D:\\mapCoord_1.txt", Encoding.Default))
            {
